...
class AutherTest {
    ...
    @Deprecated
    @Auther(username = "zhuhui")
    public void getUser() {
    }

    public static void run() throws NoSuchMethodException {
        // 1、通过反射获取info方法类
        Method method = AutherTest.class.getMethod("getUser");
        // 2、判断该方法上是否存在@Login注释
        boolean flag = method.isAnnotationPresent(Auther.class);
        if (flag) {
        ...
        for (Annotation annotation : annotations) {
            if (annotation != null && annotation instanceof Auther) {
                //如果是@Login注释，则强制转化，并调用username方法，和password方法。
                String username = ((Auther) annotation).username();
                String password = ((Auther) annotation).password();
                String description = ((Auther) annotation).description();
